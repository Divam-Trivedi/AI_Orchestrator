...
ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "http://localhost:8000,http://localhost:3000,tauri://localhost").split(",")
app.add_middleware(
    CORSMiddleware,
...
# 1. The Resource Path Helper (Essential for PyInstaller)
...
def initialize_db_location():
    ...
    if args.db_path:
        ...
        print(f"Backend: Using custom DB path: {args.db_path}")
    else:
        print("Backend: Using default DB path")

# Initialize database
initialize_db_location()
db.init_db()
